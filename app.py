...
def generate_resonance_prompt(data):
    ...
    4. ⚔️ 实战操作预案（超短线持仓）：
       - 明确给出【操作方向】和【进场区间】（给出预期核心进场价）。
       - 明确给出【精确止损价】（必须有明确的逻辑支撑，如跌破某级别布林带）。
       - ⚠️【硬性数学约束：你必须展示计算过程】：
         1) 请计算出你的单笔风险值（风险 = |核心进场价 - 止损价|）。
         2) 第一止盈位（TP1）的利润空间必须 >= 风险值的 1.5 倍。
         3) 第二止盈位（TP2）的利润空间必须 >= 风险值的 2.5 倍。
         4) 请直接在报告中写出你的计算公式，证明你给出的 TP1 和 TP2 满足上述高盈亏比要求。如果没有达到此盈亏比，请说明当前盘面“不具备高盈亏比开仓条件，建议观望”。
    """
